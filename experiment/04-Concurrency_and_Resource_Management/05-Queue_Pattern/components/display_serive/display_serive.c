...
{
    ESP_LOGI(TAG, "initializing display dev");
...
{
    ESP_LOGI(TAG, "initializing toush");
 ...
 * 
 *************************************************************/
static void ili9488_touch_deinit(void)
{
    ESP_LOGI(TAG, "deinitializing display and touch");
    //删除spi设备
    if (touch_handle != NULL) {
        ESP_ERROR_CHECK(esp_lcd_panel_io_del(touch_io_handle));
    }
...
static void lvgl_deinit(void)
{
    ...
    //释放缓冲区
    if (lv_buf_1 != NULL) {
        ESP_LOGI(TAG, "Freeing LVGL buffer 1");
        heap_caps_free(lv_buf_1);
        lv_buf_1 = NULL; // 释放后立即置空，防止悬空指针
    }
}
...
void display_task(void *arg)
{
    ...
    sensor_data_packet_t rx_packet;
    //硬件初始化
    ...
    //数据显示ui
    lv_obj_t * roll_label = lv_label_create(lv_scr_act());
    lv_obj_t * pitch_label = lv_label_create(lv_scr_act());
    lv_obj_align(roll_label, LV_ALIGN_CENTER, 0, -20);
    lv_obj_align(pitch_label, LV_ALIGN_CENTER, 0, 20);
    lv_label_set_text(roll_label, "Roll: 0.00");
    lv_label_set_text(pitch_label, "Pitch: 0.00");
    
    while(1) {
        if (xQueueReceive(queue, &rx_packet, portMAX_DELAY) == pdTRUE) {
            // 2. 根据数据类型处理 (虽然这里只有 MPU 数据)
            if (rx_packet.type == DATA_TYPE_MPU6050) {
                
                // 更新 UI
                char buf[32];
                sprintf(buf, "Roll: %.2f", rx_packet.roll);
                lv_label_set_text(roll_label, buf);
                
                sprintf(buf, "Pitch: %.2f", rx_packet.pitch);
